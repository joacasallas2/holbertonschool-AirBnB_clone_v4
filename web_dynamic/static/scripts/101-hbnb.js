  ...
  $.ajax({
    ...
    success: function (places) {
      ...
      $(".reviews span").on("click", function () {
        const articleVariable = $(this).closest("article");
        const listReviews = articleVariable.find(".reviews ul");
        if ($(this).text() === "show") {
          $(this).text("hide");
          placeReviews(articleVariable.data("id"), articleVariable);
        } else {
          $(this).text("show");
          listReviews.empty();
        }
...
function placeReviews(placeId, articlevariable) {
  const listReviews = articlevariable.find(".reviews ul");
  listReviews.empty();
  $.ajax({
    type: "get",
    ...
    success: function (reviews) {
      for (const review of reviews) {
        const reviewHtml = `
            <li>
              <h3>From ${review.user_first_name ?? "Unknown"} ${
          review.user_last_name ?? ""
        } the ${review.created_at}</h3>
              <p>${review.text}</p>
            </li>`;
        listReviews.append(reviewHtml);
      }
    },
    error: function (xhr, status, error) {
      console.error("connection error: ", status, error);
